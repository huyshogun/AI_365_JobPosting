...
def check_dk_2(dict):
    ...
    if usc_address == 'toàn quốc' or usc_address == 'tuyển dụng toàn quốc':
        print('Bản tin không được phép có địa chỉ ở toàn quốc')
    if 'toàn quốc' in usc_address:
        usc_address = usc_address.replace('toàn quốc', '')
    if 'tuyển dụng toàn quốc' in usc_address:
        usc_address = usc_address.replace('tuyển dụng toàn quốc', '')
    ...
    if usc_address != '':
        if addr != remove_accent(addr):
            ...
            if 'và' in addr or 'hoặc' in addr:
                count = count + 1
                print('có nhiều địa chỉ chi tiết')
                count = 2
        else:
            print('không có địa chỉ công ty')
            count = 3
    else:
        print('không có địa chỉ công ty')
        count = 3
...
def check_dk_3(dict):
    ...
    count = 0
    new_hinh_thuc_text = new_hinh_thuc_text.lower()
    #if new_hinhthuc != str(1) or new_hinh_thuc_text != "Toàn thời gian cố định":
    if new_hinh_thuc_text == "bán thời gian" or new_hinh_thuc_text == "bán thời gian tạm thời" or new_hinh_thuc_text == "việc làm từ xa" or new_hinh_thuc_text == 'part-time' or new_hinh_thuc_text == 'học việc':
        count = count + 1
    return count
...
def check_dk_4(dict):
    ...
    pattern = '[0-9]{2}k/h'
    if pattern in new_mota.lower():
       count = 1
    if pattern in new_quyenloi.lower():
       count = 1
    if pattern in new_title.lower():
       count = 1
       print("Lương có trong tiêu đề")
...
def check_dk_5(dict):
    ...
    if money[0] == '0':
        if new_money_str == 'Thỏa thuận':
            ...
            count = count + 1
            print('lương 1 đầu trong money str')
    else:
        if (money_ql[0] == '0' and money_mt[0] == '0' and money_yc[0] == '0'):
            if (mon_ql[0] != '0' or mon_mt[0] != '0' or mon_yc[0] != '0'):
                ...
                print('không có lương trong nội dung')
                count = count + 0
        else:
            count = count + count_ql + count_mt + count_yc
...
def check_dk_6(dict):
    ...
    if new_name_qh == '' or new_name_cit[0] == '':
            count = count + 1
            print('không có quận huyện, không có tỉnh thành')
    else:
        addr = addr.replace('–', ',')
        addr = addr.replace('-', ',')
        addr = addr.replace(' , ', ',')
...
def check_dk_9(dict):
    ...
    for pat in pats:
        if re.search(pat, back_title) is not None:
...
def check_dk_10(dict):
    ...
    if (len(new_mota)+len(new_yeucau)+len(new_quyenloi)) != 0:
        ...
        ratio = 0.06
        print('khong co mota quyenloi yeucau')
        status = 2
    ...
    print('tile:', ratio)
    if ratio < 0.05:
        return 0, new_mota, new_quyenloi, new_yeucau
    else:
        status = 1
        for special in text_special:
           ...
           if (len(new_mota)+len(new_yeucau)+len(new_quyenloi)) != 0:
              ...
              if(ratio < 0.05):
               ...
               status = 2
               break
    return status, new_mota, new_quyenloi, new_yeucau
...
class DataModel:
    def __init__(self, result, new_id, message, item, fixed_text, dictionary):
        self.result = result
        self.new_id = new_id
        self.message = message
        self.item = item
